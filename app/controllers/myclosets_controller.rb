class MyclosetsController < ApplicationController
  ...
  before_action :set_user_clothe, only: %i[edit destroy select publish]
  def index
  ...
  def create
    ...
    if @brand_name.save
      ...
      @user_clothe = UserClothe.new(user_id: current_user, clothe_id: clothe)
      @user_clothe.save
      # 背景削除の処理
  ...
  def search
    @results = @q.result.includes(:clothe, clothe: [:brand_name]).where(user_clothes: {user_id: current_user})
    @selected_count = UserClothe.selected(current_user).count
  end
  ...
  def publish
    @user_clothe.update(publish_params)
    redirect_to myclosets_path
  end
